...
```mermaid
pie title Product Tier Distribution Target (% Revenue)
    "Tier 2 Curated Mid 60%" : 60
    "Tier 1 Essential 25%" : 25
    "Tier 3 Statement 12%" : 12
    "Tier 4 Heritage Bespoke 3%" : 3
```
...
```mermaid
quadrantChart
    title Price Positioning vs Competitor
    x-axis Low Price --> High Price
    y-axis Low Curation --> High Curation
    quadrant-1 Premium Curated
    quadrant-2 Premium Generic
...
```mermaid
flowchart TD
    A[Customer request discount] --> B{Within program?}
    
    B -->|Yes ≤10%| C[MA apply]
    B -->|Yes 10-15%| D[Door Expert apply]
    B -->|Yes >15%| E[Matthew approve]
    B -->|No| F[Decline politely]
    
    C --> G[Document CRM]
    D --> G
    E --> G
    F --> H[Premium hangat decline]
